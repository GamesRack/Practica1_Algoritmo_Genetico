...
def mutar(crom: List[int], p: float) -> List[int]:
    #Copiamos el cromosoma original (para no modificarlo directamente)
    c = crom[:]  
    #Decidir si mutar según la probabilidad 
    if random.random() < p:
         #Elegimos un gen aleatorio
        i = random.randrange(NUM_productos) 
        if random.random() < 0.5:
            #RESET
            c[i] = random.randint(0, MAX_productos)
        else:
            #INCREMENTO/DECREMENTO
            cambio = random.choice([-1, 1])   #elige -1 o +1
            c[i] = max(0, min(MAX_productos, c[i] + cambio))

    #Pasamos a verificar y reparar el cromosoma para asegurar que cumple límites y restricciones
    return reparar(c)
